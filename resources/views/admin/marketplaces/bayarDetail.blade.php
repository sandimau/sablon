                    ...
                    <table class="table table-striped table-hover">
                        <thead>
                            <tr>
                                ...
                                <th>bayar</th>
                                <th>
                                    <a href="{{ request()->fullUrlWithQuery(['sort' => 'potongan', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}"
                                       class="text-decoration-none text-dark">
                                        Potongan
                                        @if(request('sort') == 'potongan')
                        ...
                        <tbody>
                            @foreach ($data as $order)
                                <tr data-entry-id="{{ $order->id }}">
                                    ...
                                    <td>{{ number_format($order->total - $order->bayar, 0, ',', '.') }}</td>
                                    <td>
                                        @if($order->total > 0)
                                            {{ number_format((($order->total - $order->bayar) / $order->total) * 100, 1) }}%
                                        @else
                                            0%
                                        @endif
                                    </td>
